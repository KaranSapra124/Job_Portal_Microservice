    ...
    try {
        ...
        if (!skills) {
            return res.status(400).json({
                message: "Skills are not provided!"
            });
        }
        const prompt = `
        ...
        try {
            ...
            try {
                ...
                return res.status(500).json({
                    message: "Ai response is not a valid JOSN",
                    rawResponse: response?.text
                });
            }
            return res.status(201).json({ message: "AI Message Generated", jsonResponse });
        }
        catch (err) {
            return res.status(500).json({
                message: err.message
...
`;
        ...
        try {
            ...
            return res.status(201).json({ message: "Your resume feedback is this", jsonResponse });
        }
        catch (err) {
            return res.status(500).json({
                message: err.message
